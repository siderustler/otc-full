...
local conditions = { -- always add new conditions at the bottom
    ...
    "Dangerous Mobs more than:" -- spinbox 19
}
...
local function getDangerousMonsters()
    local dangerousMobs = {"Feral Sphinx", "Sphinx", "Manticore", "Lamassu", "Crypt Warden"}
    local monsters = 0
    local spectators = getSpectators()
    for i, spec in ipairs(spectators) do
        if spec:isMonster() and table.find(dangerousMobs, spec:getName()) then
          monsters = monsters + 1
        end
      end
    return monsters
