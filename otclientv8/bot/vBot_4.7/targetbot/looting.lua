...
onTextMessage(function(mode, text)
  ...
  if string.find(text:lower(), "you are not the owner") or string.find(text:lower(), "you cannot loot a corpse") then -- if we are not the owners of corpse then its a waste of time to try to loot it
    table.remove(TargetBot.Looting.list, storage.extras.lootLast and #TargetBot.Looting.list or 1)
  end
end)
